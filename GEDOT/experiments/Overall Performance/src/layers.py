...
class AttentionModule(torch.nn.Module):
    ...
    def forward(self, embedding):
        ...
        #embedding：n x d
        #weight_matrix: d x d
        #每列求mean得到 d个元素的行向量size是d
        global_context = torch.mean(torch.matmul(embedding, self.weight_matrix), dim=0)
        transformed_global = torch.tanh(global_context)
        #把transformed_global扩张成2D (d,1)张量
        sigmoid_scores = torch.sigmoid(torch.mm(embedding, transformed_global.view(-1, 1)))
        #sigmoid_scores: n x 1
        representation = torch.mm(torch.t(embedding), sigmoid_scores)
        # representation: d x 1
...
class SinkhornLayer(nn.Module):
    ...
    def __init__(self, max_iter: int=10, tau: float=1e-1, epsilon: float=1e-8):
    ...
    def forward(self, cost_matrix):
        ...
        u = torch.ones_like(prob1)/n1
        v = torch.ones_like(prob2)/n2        
        for _ in range(self.max_iter):
            u = prob1 / ((K * v.unsqueeze(0)).sum(1) + self.epsilon)
            v = prob2 / ((K * u.unsqueeze(1)).sum(0) + self.epsilon)
        match = K * u.unsqueeze(-1) * v.unsqueeze(-2)
        return match
