...
from layers import AttentionModule, TensorNetworkModule, SinkhornLayer, MatchingModule, GraphAggregationLayer
...
class GedGNN(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        if self.args.gnn_operator == 'gcn':
            ...
            self.convolution_2 = GCNConv(self.args.filters_1, self.args.filters_2)
            self.convolution_3 = GCNConv(self.args.filters_2, self.args.filters_3)
        elif self.args.gnn_operator == 'gin': #GIN网络
            nn1 = torch.nn.Sequential(
                torch.nn.Linear(self.number_labels, self.args.filters_1),
                torch.nn.ReLU(),
                torch.nn.Linear(self.args.filters_1, self.args.filters_1),
    ...
    def forward(self, data):
        ...
        features_1 = data["features_1"]
        features_2 = data["features_2"]
        #after GIN得到feature矩阵 n_1 x d 和 n_2 x d
        abstract_features_1 = self.convolutional_pass(edge_index_1, features_1)
        abstract_features_2 = self.convolutional_pass(edge_index_2, features_2)
        #内含MLP
        ...
        #两个matrix大小是 n_1 x n_2
        # calculate ged using map_matrix
        m = torch.nn.Softmax(dim=1)#按行softmax
...
class TestGED(torch.nn.Module):
    
    def __init__(self,args,number_of_labels):
        super(TestGED,self).__init__()
        self.args = args
        self.number_labels = number_of_labels
        self.setup_layers()
    ...
    def setup_layers(self):
        ...
        if self.args.gnn_operator == 'gcn':
            ...
            self.convolution_2 = GCNConv(self.args.filters_1, self.args.filters_2)
            self.convolution_3 = GCNConv(self.args.filters_2, self.args.filters_3)
        elif self.args.gnn_operator == 'gin': #GIN网络 3层
            nn1 = torch.nn.Sequential(
                torch.nn.Linear(self.number_labels, self.args.filters_1),
                torch.nn.ReLU(),
                torch.nn.Linear(self.args.filters_1, self.args.filters_1),
