...
class Trainer(object):
    ...
    def transfer_data_to_torch(self):
        ...
        for g in self.graphs:
            ...
            edge = edge + [[x, x] for x in range(g['n'])] #自环
        ...
        self.gid = gid
        self.gn = [g['n'] for g in self.graphs] #顶点数 数组 
        self.gm = [g['m'] for g in self.graphs] # 边数 数组
        for i in range(n):
    ...
    def delta_graph(g, f, device):
        ...
        random.shuffle(edge)#边集重排
        m = len(edge)
        ged = random.randint(1, 5) if n <= 20 else random.randint(1, 10)
        del_num = min(m, random.randint(0, ged))
        edge = edge[:(m - del_num)]  # the last del_num edges in edge are removed 删掉后面的边
        add_num = ged - del_num
    ...
    def fit(self):
        ...
        with open(self.args.abs_path + self.args.result_path + 'results_'+self.args.model_name+'.txt', 'a') as f:
